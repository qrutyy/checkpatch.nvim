...
local function shell_escape_dquote(str) return (str or ""):gsub('"', '\\"') end
...
function M.run(cfg)
    ...
    if file == "" then
        if not cfg.quiet then
            vim.notify("Buffer is empty. Running in current dir",
                       vim.log.levels.INFO)
            cfg.filem = true
        end
    end
    ...
    if cfg.diff then
        ...
        if not repo_root then
            if not cfg.quiet then
                vim.notify(
                    "checkpatch: not a git repo; falling back to file mode",
                    vim.log.levels.WARN)
            end
            handle = io.popen("perl " .. checkpatch_path .. " " .. opts ..
                                  "--file " .. file)
        else
            local base = cfg.diff_base or "HEAD"
            local esc_repo = '"' .. shell_escape_dquote(repo_root) .. '"'
            local esc_file = '"' .. shell_escape_dquote(file) .. '"'
            local cmd = "git -C " .. esc_repo .. " diff " .. base .. " -- " ..
                            esc_file .. " | perl " .. checkpatch_path .. " " ..
                            opts .. "-"
            handle = io.popen(cmd)
        end
    else
    ...
    if not handle then print("Error opening the file") end

    local result = handle:read("*a")
    handle:close()

    if cfg.log then write_log(result) end
